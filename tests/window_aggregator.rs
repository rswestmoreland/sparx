...
fn apply_line_resolves_features_updates_meta_and_entities() {
    ...
    let mut acc =
        WindowAccumulatorV1::new_v1("dev01", window_start_ts, 7, 60, line_ts, base_caps()).unwrap();
    ...
    let src_bucket = dict
        .lookup_feature_id_v1("SourceIp_net@10.2.3.0/24")
        .unwrap();
...
fn apply_line_returns_different_window_without_mutation() {
    ...
    let result = acc
        .apply_line_v1(start + 60, start + 61, 55, &line, &mut dict)
        .unwrap();
    assert_eq!(
        result,
...
fn window_caps_drop_words_plaintext_shapes_and_general_features_deterministically() {
    ...
    let result = acc
        .apply_line_v1(start, start + 1, 99, &line, &mut dict)
        .unwrap();
    let applied = match result {
        WindowApplyLineResultV1::Applied(v) => v,
        other => panic!("unexpected apply result: {:?}", other),
...
fn checkpoint_writes_are_ordered_and_restore_roundtrips_state() {
    ...
    let window_start_ts = align_window_start_ts_v1(line_ts, 60).unwrap();
    let mut acc =
        WindowAccumulatorV1::new_v1("dev01", window_start_ts, 42, 60, line_ts, base_caps())
            .unwrap();
    ...
    let _ = acc
        .apply_line_v1(line_ts, line_ts + 1, 321, &line, &mut dict)
        .unwrap();
    ...
    assert_eq!(
        writes[2].key,
        key_tenant_window_row_ent_srcip_v1("dev01", 42)
    );
    assert_eq!(
        writes[3].key,
        key_tenant_window_row_ent_dstip_v1("dev01", 42)
    );
    assert_eq!(
        writes[4].key,
        key_tenant_window_row_ent_userid_v1("dev01", 42)
    );
    assert_eq!(
        writes[5].key,
    ...
    assert_eq!(
        writes[6].key,
        key_tenant_window_row_ent_host_v1("dev01", 42)
    );
...
fn finalized_row_snapshot_matches_active_state() {
    ...
    let _ = acc
        .apply_line_v1(line_ts, line_ts + 1, 222, &line, &mut dict)
        .unwrap();
...
fn finalize_and_advance_rolls_to_next_empty_window_and_updates_active_last() {
    ...
    let _ = acc
        .apply_line_v1(line_ts, line_ts + 1, 111, &line, &mut dict)
        .unwrap();
    ...
    assert_eq!(
        next.active_v1().active_window_start_ts,
...
fn finalize_and_advance_rejects_overlapping_or_misaligned_next_window_start() {
    ...
    let overlap = acc
        .finalize_and_advance_v1(start + 30, start + 31)
        .unwrap_err();
    assert_eq!(
        overlap,
    ...
    let misaligned = acc
        .finalize_and_advance_v1(start + 61, start + 62)
        .unwrap_err();
    assert_eq!(
        misaligned,
        sparx::window::WindowErrorV1::InvalidNextWindowStart {
...
fn finalize_and_advance_rejects_window_id_overflow() {
    let start = Utc
        ...
        .timestamp();
    let acc =
        WindowAccumulatorV1::new_v1("dev01", start, u64::MAX, 60, start, base_caps()).unwrap();

    let err = acc
        .finalize_and_advance_v1(start + 60, start + 61)
        .unwrap_err();
    assert_eq!(
        err,
...
fn apply_line_is_atomic_when_dictionary_insert_fails() {
    ...
    let err = acc
        .apply_line_v1(start, start + 1, 88, &line, &mut dict)
        .unwrap_err();
    assert_eq!(
        err,
