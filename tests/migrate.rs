...
use sparx::db::{
    GlobalSchemaStateV1, GlobalTenantRecordV1, TenantMigrateJournalEntryV1, TenantSchemaStateV1,
};
use sparx::runtime::{
...
fn migrate_current_schema_is_noop_v1() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tenant_entries =
        runtime.with_tenant_db_v1("tenant-a", 301, |db| db.scan_migrate_journal_entries_v1())?;
    assert!(tenant_entries.is_empty());
    Ok(())
}

#[test]
fn migrate_missing_schema_initializes_global_and_tenant_v1(
) -> Result<(), Box<dyn std::error::Error>> {
    let cfg = temp_cfg_v1();
    let mut runtime = SparxRuntimeV1::open_from_config_v1(&cfg)?;
    seed_tenant_record_v1(&runtime, "tenant-a", 0)?;
    ...
    assert_eq!(
        Some(GLOBAL_SCHEMA_VERSION_CURRENT_V1),
        runtime.read_global_schema_state_v1()?.map(|s| s.version)
    );
    let tenant_state =
        runtime.with_tenant_db_v1("tenant-a", 2001, |db| db.read_schema_state_v1())?;
    assert_eq!(
        Some(TENANT_SCHEMA_VERSION_CURRENT_V1),
        tenant_state.map(|s| s.version)
    );
    ...
    let tenant_entries =
        runtime.with_tenant_db_v1("tenant-a", 2002, |db| db.scan_migrate_journal_entries_v1())?;
    assert_eq!(
...
fn migrate_tenant_downgrade_refusal_disables_tenant_v1() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tenant_entries =
        runtime.with_tenant_db_v1("tenant-a", 3001, |db| db.scan_migrate_journal_entries_v1())?;
    assert!(tenant_entries.is_empty());
    let global_entries = runtime.scan_global_migrate_journal_entries_v1()?;
...
fn migrate_global_downgrade_refusal_route_exits_one_v1() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(r
        .msg_stderr
        .unwrap()
        .contains("global schema version 2 is newer than binary schema 1"));
    Ok(())
}
...
fn migrate_all_is_deterministic_and_skips_terminated_v1() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert_eq!(
        SchemaMigrateOutcomeKindV1::Initialized,
        result.tenants[0].outcome
    );
    assert_eq!(
        SchemaMigrateOutcomeKindV1::Initialized,
        result.tenants[1].outcome
    );
    assert_eq!(
        SchemaMigrateOutcomeKindV1::SkippedTerminated,
        result.tenants[2].outcome
    );
    Ok(())
}
...
fn migrate_all_partial_route_returns_exit_six_v1() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stderr.contains("tenant-b: tenant tenant-b schema version 2 is newer than binary schema 1")
    );
...
fn migrate_tenant_missing_route_returns_exit_one_v1() {
    ...
    assert!(r
        .msg_stdout
        .unwrap()
        .contains("tenant_outcome: tenant_not_found"));
