...
- Phase 15b and later persist secondary `alert_idx_*` keys alongside the primary alert object.
...
Query strategy:
- Phase 15b persists secondary indexes for deterministic restart continuity.
- Phase 15c activates the `alert_idx_time` path for list/search/export candidate selection when the time index is complete for the tenant DB.
...
- Phase 15d activates structured category/entity filters on the CLI surface.
